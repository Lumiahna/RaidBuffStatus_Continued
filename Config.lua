...
local L = vars.L
RBS_svnrev["Config.lua"] = select(3,string.find("$Revision: 723 $", ".* (.*) .*"))
	...
	args = {
		...
		reporting = {
			...
			args = {
				...
				hidewhispers = {
					type = 'toggle',
					name = L["Hide outgoing whispers"],
					desc = L["Hide outgoing whispers"],
					order = 12.5,
					get = function(info) return profile.hidewhispers end,
					set = function(info, v)
						profile.hidewhispers = v
					end,
				},
				abouttorunout = {
					type = 'range',
