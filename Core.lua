...
RBS_svnrev = {}
RBS_svnrev["Core.lua"] = select(3,string.find("$Revision: 727 $", ".* (.*) .*"))
...
--	[201562] = { category="Feast", itemid=133578, limit=35, },   -- Hearty Feast
...
addon.cons_exceptions = {
	...
	[188534] = { 100, SpellName(188512) },  -- Felmouth Frenzy, treat it as 100 stat food for now
}
...
function addon:StatLevelCalc(isfood, spellId, unitid, b)
	...
	if not mainstat then
   	...
   	if mainstat == ITEM_MOD_STAMINA_SHORT and
...
function addon:BeginWhispers()
  addon.wspcnt = 0
  addon.wspresult = wipe(addon.wspresult or {})
end
function addon:EndWhispers()
  if addon.wspcnt > 0 then
    local wcnt = addon.wspcnt
    local pcnt = 0
    local resultstr = ": "
    for p,_ in pairs(addon.wspresult) do
      resultstr = resultstr..(pcnt > 0 and ", " or "")..p
      pcnt = pcnt + 1
    end
    addon:ScheduleTimer(function() -- use a delay, so the print will usually appear last
    	addon:Print(string.format(L["Sent %d whispers to %d players"], wcnt, pcnt)..resultstr)
    end, 0.5)
  end
end
...
function addon:Say(msg, player, prepend, channel)
	...
	while #msg > 0 do
	  ...
	  elseif player then
		...
		addon.rbswspfilt[str] = GetTime()
		addon.wspcnt = (addon.wspcnt or 0) + 1
		if addon.wspresult then
			addon.wspresult[player] = true
		end
...
function addon:COMBAT_LOG_EVENT_UNFILTERED(event, timestamp, subevent, hideCaster, 
	...
	if (subevent == "UNIT_DIED" and band(dstflags,COMBATLOG_OBJECT_TYPE_PLAYER) > 0) or 
	   (spellID == 27827 and subevent == "SPELL_AURA_APPLIED") then -- Spirit of Redemption
		...
		if subevent == "UNIT_DIED" and unit.class == "PRIEST" and unit.spec == 2 then -- Holy
...
function addon:MisdirectionEventLog(srcname, spellname, dstname)
	...
	if profile.misdirectionsound then
		PlaySoundFile("Sound\\Doodad\\ET_Cage_Close.ogg", "Master")
	end
end
...
function addon:CCBreakSay(msg, typeoftaunt)
	if typeoftaunt == "ccwarntank" then
		if profile.ccwarntanksound then
			PlaySoundFile("Sound\\Creature\\Ram\\RamPreAggro.ogg", "Master")
		end
	...
	elseif typeoftaunt == "ccwarnnontank" then
		if profile.ccwarnnontanksound then
			PlaySoundFile("Sound\\Creature\\Sheep\\SheepDeath.ogg", "Master")
		end
	...
	if profile.aiwbnfriends and BNFeaturesEnabledAndConnected() then
		for i=1, BNGetNumFriends() do
		   ...
		   if isOnline then
			...
			local _, toonName, client, realmName, realmId, faction = BNGetFriendGameAccountInfo(i,j)
		...
		for i=1, BNGetNumFriends() do
		   ...
		   if isOnline then
		     for j=1, BNGetNumFriendGameAccounts(i) do
		        local _, toonName, client, realmName, realmId, faction = BNGetFriendGameAccountInfo(i,j)
			if client == "WoW" then
				--local _,name,_,realm = BNGetGameAccountInfo(pID)
